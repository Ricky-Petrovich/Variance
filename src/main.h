...
static const int LAST_POW_BLOCK = 8000000;
...
static const int STAKE_TARGET_SPACING = 480;
...
public:
    ...
    {
        // VARY: the coin stake transaction is marked with the first output empty
    ...
    */
    ...
    int64 GetMinFee(unsigned int nBlockSize=1, bool fAllowFree=false, enum GetMinFee_mode mode=GMF_BLOCK) const
    {
        ...
        if (fAllowFree)
        {
            if (nBlockSize == 1)
            {
                // Transactions under VARY are free
    ...
    bool GetCoinAge(CTxDB& txdb, uint64& nCoinAge) const;  // VARY: get transaction coin age
...
public:
    ...
    // VARY: block signature - signed by coin base txout[0]'s owner
    ...
    // VARY: two types of block: proof-of-work or proof-of-stake
    ...
    // VARY: get max transaction timestamp
    ...
    bool AcceptBlock();
    bool GetCoinAge(uint64& nCoinAge) const; // VARY: calculate total coin age spent in block
    bool SignBlock(const CKeyStore& keystore);
    bool CheckBlockSignature() const;
    unsigned int GetStakeEntropyBit() const; // VARY: entropy bit for stake modifier if chosen by modifier
...
public:
    ...
    unsigned int nFile;
    unsigned int nBlockPos;
    CBigNum bnChainTrust; // VARY: trust score of block chain
    int nHeight;
    int64 nMint;
    int64 nMoneySupply;

    unsigned int nFlags;  // VARY: block index flags
    enum  
    {
