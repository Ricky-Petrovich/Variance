...
// VARY: optional setting to unlock wallet for block minting only;
...
// VARY: total coins staked (non-spendable until maturity)
       ...
       BOOST_FOREACH(const CWalletTx* pcoin, vCoins)
       {
            ...
            for (unsigned int i = 0; i < pcoin->vout.size(); i++)
            {
                ...
                if (pcoin->nTime > nSpendTime)
                    continue;  // VARY: timestamp must not exceed spend time
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            loop
            {
                ...
                // VARY: sub-cent change is moved to fee
                ...
                if (nChange > 0)
                {
                    ...
                    if (!GetBoolArg("-avatar")) // VARY: not avatar mode
                    {
...
// VARY: create coin stake transaction
...
// VARY: check 'spent' consistency between wallet and txindex
// VARY: fix wallet spent state according to txindex
...
// VARY: disable transaction (only for coinstake)
